...
@Directive({
  selector: 'img[nighthawk-image-loader]',
  standalone: true,
})
export class NighthawkImageLoaderDirective implements OnInit, OnChanges {
  ...
  private setAspectRatio(): void {
    const aspectRatio =
      this.width && this.height ? `${this.width} / ${this.height}` : '16 / 9';
  ...
  private loadImage(): void {
    ...
    const loadListener = this.renderer.listen(
      this.el.nativeElement,
      'load',
      () => {
        this.onLoad();
    ...
    const errorListener = this.renderer.listen(
      this.el.nativeElement,
      'error',
      () => {
        this.onError();
